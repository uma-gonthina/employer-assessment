...
const fields = ref(["name", "phone_number", "email", "address", "status"]);
...
const emitData = (sale: any, index: any, value: any) => {
  if (value == "edit") {
    editProject.value = JSON.parse(JSON.stringify(sale));
    edit.value = true;
    return;
  }
  emit("emitData", { sale: sale, value: value, index: index });
};
</script>
